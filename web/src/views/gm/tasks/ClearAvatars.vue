<template>
  <div>
    <panel-title title="删除时装"></panel-title>
    ...
    <div class="tc-step-border">
      <div class="l">
        ...
        <panel-step num="3" title="执行删除（切换角色后生效）" is-last>
          <el-button type="danger" :disabled="!characNo" @click="doClear" size="small">清空时装背包</el-button>
        </panel-step>
      </div>
    </div>
  </div>
...
import { clearAvatars } from '@/api/gm/task'
...
const doClear = async () => {
  ...
  await confirmWarning('确认清空该角色时装背包？')
  await clearAvatars(characNo.value)
