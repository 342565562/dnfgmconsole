<template>
  <div>
    <el-row>
      <span class="text-warning sm">注：每30分钟自动清理全服邮箱</span>
    </el-row>
    <el-row>
    ...
    <el-table v-loading="loading" :data="data" ref="tableRef" border @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="play_1" label="邮件ID" width="180"></el-table-column>
      <el-table-column prop="play_1_trigger" label="邮件信息" width="180">
        <template #default="scope">
          <span>{{ scope.row.play_1_trigger ? '已收邮件' : '邮件附件' }}</span>
        </template>
      </el-table-column>
